...
# Load data
orth  = loadExamples('../../aae/raw/3k/orth.csv',  simplify=True)
phon  = loadExamples('../../aae/raw/3k/phon.csv',  simplify=True)
with open('../../aae/raw/3k/words.csv', 'r') as f:
...
def main():
    ...
    with open('candide_error_on_corpus.txt', 'w') as f:
        for e in learner.error_on_corpus:
            f.write("{loss:.4f}\n".format(loss=e))

    with open('candide_training_sequence.txt', 'w') as f:
        for e in learner.training_sequence:
            f.write("{example:d}\n".format(example=e))
